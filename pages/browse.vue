  ...
  <main v-if="baseAikus && aikusForFeed && aikuImgMap.size > 0">
    ...
    <div class="fixed bottom-0 px-4 pb-6 flex items-center justify-between w-full" :style="'opacity:' + opacity + ';'">
      ...
      <div class="flex items-center space-x-4">
        ...
        <!-- download -->
        <button @click="copyToClipboard(true)" type="button" class="inline-flex items-center rounded-md border border-transparent bg-black/50 px-3 py-1.5 text-xs font-medium leading-4 text-white shadow-sm hover:bg-black/75 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-hover-300">
          Share
...
function copyToClipboard(showNoti: boolean) {
  const url = config.public.baseUrl + '/aikus/' + curAikuId.value
