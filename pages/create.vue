<template>
  <WideContainer>
    ...
    <div v-if="pageState === 'edit'" class="grid grid-cols-1 md:grid-cols-2 gap-y-10 gap-x-8 h-[90vh]">
      <div class="relative mx-auto md:max-w-xl">
        ...
        <div class="md:absolute bottom-0 w-full mt-6 md:mt-0 flex space-x-4">
          ...
          <SubmitButton v-if="user" class="w-full" color="violet" submit-text="Save" :is-valid-state="true"
            :submit-loading="saveLoading" submit-loading-text="saving" :svg="save" @submit="saveAiku()" />

          <SubmitButton v-else class="w-full" color="violet" submit-text="Sign in to save" :is-valid-state="false"
            :submit-loading="false" :submit-loading-text="loadingStatus" :svg="save" />
        </div>
      </div>
      <CreateAikuImgPane class="h-[70vh] md:h-full" />
    </div>
...
import { rocket, save } from '../assets/icons'
